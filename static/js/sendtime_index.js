...
$("#rulesubmit").click(function(){
      var username = $(".signup_mid #id_username").val();
      var vcode = $(".signup_mid #id_vcode").val();
      if($('.signup_mid #id_vcode').hasClass('inputxt Validform_error') || !vcode){
          alert("请输入正确的验证码！");
      }
      else{
          if($('.signup_mid #id_username').hasClass('inputxt Validform_error') || !username){
            alert("请输入手机号！");
          }
          else{

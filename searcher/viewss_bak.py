...
def qq_is_first(request):
    ...
    if request.method == 'POST':
        ...
        if openid != "" and accesstoken != "" and username == "" and email == "": #username和email都为空的处理
            tl = ThirdLogin.objects.filter(openId=openid) #通过openid在数据库中查找
            if tl.exists():
                if tl[0].qqFlag == "1":
                    u = User.objects.filter(id=(UserInformation.objects.filter(id=tl[0].userInfo_id)[0].user_id))
                    username = u[0].username  #获取username
                    resp = 1
                else:
                    resp = 0
            else:
                resp = 0
        elif username != "" and email != "" and openid != "" and accesstoken != "": #有usename和email的处理逻辑
            u = User.objects.filter(username=username)  #通过username在数据库中查找
            if u.exists():
                resp = 2
            else:
                new_user = User.objects.create_user(username=username, password="openid", email=email) #如果是qq账号第一次登陆，保存在user数据库中
                new_user.save()
                u = UserInformation(user=new_user, photo_url=url) #保存用户信息到userinformation数据库模型中
                u.save()
                tl = ThirdLogin(openId=openid, accessToken=accesstoken, qqFlag=0) #保存用户信息到ThirdLogin数据库模型中
                tl.userInfo = u
                tl.qqFlag = 1
                tl.save()
                resp = 1
        ...
        if resp == 1:
            ...
            return render_to_response("register.html", {'form': form, 'error': error, 'openid': openid}, context_instance=RequestContext(request)) #返回注册页面
    else:
...
def wb_is_first(request):
    ...
    if request.method == 'POST':
        ...
        nickname = request.POST.get('nickname', '')
        url = request.POST.get('url', '')
        if wbid != "" and username == "" and email == "": #通过微博账号查询
            ...
            if tl.exists():
                ...
                resp = 0
        elif username != "" and email != "": #通过username和email查找
...
def initData(code):
    httpClient = None
    try:
        httpClient = httplib.HTTPSConnection('api.weixin.qq.com', timeout=30) #连接服务器
        httpClient.request('GET', '/sns/oauth2/access_token?appid=wx5705d3f981443fb8&secret=46f37dba35e5b57b5b98cc2437296f6b&code='+code+'&grant_type=authorization_code') #发送服务器请求
        response = httpClient.getresponse()
        if response.status == 200 and response.reason == 'OK': #返回结果处理
            url = response.read()
            print "url:|||:", url
            time = json.loads(url)
        return time
...
def wxcheck(request):
    ...
    if request.method == 'GET':
        response = HttpResponse(checkSignature(request)) #检测是否已经登陆
        return response
    else:
...
def wx_bid_detail(request, objectid):
    ...
    connection = MySQLdb.connect(host="ddbid2015.mysql.rds.aliyuncs.com", user="django", passwd="ddbid_django1243", db="ddbid_db")#连接数据库
